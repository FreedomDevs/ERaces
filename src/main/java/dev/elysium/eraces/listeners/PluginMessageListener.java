...
public class PluginMessageListener implements org.bukkit.plugin.messaging.PluginMessageListener {
    ...
    private void handleStart(Player player) {
        ...
        if (cfg.isCastFeedback()) {
            ChatUtil.sendAction(player, "<green>Начат ввод комбинации...");
        }
    }
    ...
    private void handleKey(Player player, String payload) {
        ...
        if (s == null) {
            ...
            if (cfg.isCastFeedback()) ChatUtil.sendAction(player, "<red>Сначала начни ввод (start cast).");
            return;
        }

        if (s.isExpired()) {
            sessions.remove(player.getUniqueId());
            if (cfg.isCastFeedback()) ChatUtil.sendAction(player, "<red>Время ввода прошло — начни заново.");
            return;
        }

        if (s.getKeys().length() >= cfg.getCastMaxLength()) {
            if (cfg.isCastFeedback()) ChatUtil.sendAction(player, "<red>Комбинация слишком длинная.");
            return;
        }
        ...
        if (cfg.isCastFeedback()) {
            ChatUtil.sendAction(player, "<yellow>Текущая комбинация: <white>" + s.getKeys());
        }
    }
    ...
    private void handleEnd(Player player, String payload) {
        CastingSession s = sessions.get(player.getUniqueId());
        if (s == null) {
            if (cfg.isCastFeedback()) ChatUtil.sendAction(player, "<red>Нечего завершать — начни ввод комбинации сначала.");
        ...
        if (s.isExpired()) {
            sessions.remove(player.getUniqueId());
            if (cfg.isCastFeedback()) ChatUtil.sendAction(player, "<red>Время ввода истекло — комбинация сброшена.");
            return;
        }
        ...
        if (collected.isEmpty()) {
            if (cfg.isCastFeedback()) ChatUtil.sendAction(player, "<red>Комбинация пуста.");
            return;
        }
        ...
        try {
            ...
            e.printStackTrace();
            if (cfg.isCastFeedback()) ChatUtil.sendAction(player, "<red>Ошибка при активации способности.");
        }
    }
}
